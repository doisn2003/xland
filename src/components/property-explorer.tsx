...
import { emptyFilters, filterProperties, type Filters } from "@/data/properties";
  ...
  return (
    <section className="explorer container" id="kham-pha" aria-labelledby="explorer-title">
      <form className="search-panel" onSubmit={search} aria-label="Tìm kiếm lô đất">
        <label><span id="filter-region-label">KHU VỰC</span><select aria-labelledby="filter-region-label" value={draft.region} onChange={(e) => setDraft({ ...draft, region: e.target.value })}><option value="">Tất cả khu vực</option><option>Khánh Hòa</option><option>Đồng bằng Bắc Bộ</option></select></label>
        ...
        <label><span id="filter-setting-label">KHÔNG GIAN</span><select aria-labelledby="filter-setting-label" value={draft.setting} onChange={(e) => setDraft({ ...draft, setting: e.target.value })}><option value="">Bạn đang tìm gì?</option><option>Ven sông</option><option>Nhà vườn</option><option>Đồng quê</option></select></label>
      ...
      <div className="results-toolbar"><p aria-live="polite">{results.length} hồ sơ mẫu</p><button className="text-button" onClick={reset}>Xóa bộ lọc <span aria-hidden="true">↻</span></button></div>
